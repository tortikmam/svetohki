...
int main() {
    ...
    while (choice != 0) {
        ...
        switch (choice) {
            case 1: {
                ...
                while (count < targetCount) {
                    ...
                    for (const auto& item : listResponse["data"]) {
                        ...
                        p_data->family = p["family"]; // <-- ДОБАВИЛИ
            ...
            case 2: {
                if (!hasData) break;
                quickSort(plants_vector, 0, (int)plants_vector.size() - 1, cmpByFamilyAndName);
                isSorted = true;
                cout << "sort by family and name completed" << endl;
                break;
            }
            ...
            case 4: {
                ...
                string target_family, target_name;
                cout << "DOP search - enter family: ";
                cin.ignore(); // Очищаем буфер от предыдущего ввода меню
                getline(cin, target_family);
                cout << "DOP search - enter name_ru: ";
                getline(cin, target_name);

                // Используем обновленную функцию из dop_tree.cpp с двумя ключами
                DOPNode* found = searchDOP(rootDOP, target_family, target_name);
                // --- НАШИ ИЗМЕНЕНИЯ ЗАКАНЧИВАЮТСЯ ЗДЕСЬ ---
