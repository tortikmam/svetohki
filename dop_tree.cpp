...
DOPNode* searchDOP(DOPNode* root, const std::string& target) {
    if (root == nullptr || root->data->name_ru == target) {
        return root;
    }
    // Классический бинарный поиск: если цель меньше — идем влево
    if (target < root->data->name_ru) {
        return searchDOP(root->left, target);
    }
    return searchDOP(root->right, target);
}
